...
export const getStaticProps = async () => {
  const productPageContent = await productPage.find({
    locale: "ko-KR",
    populate: ["pageInfo", "section1", "section1.products"]
  });
  return {
    props: {
      content: productPageContent?.data
    }
  };
};
...
export default function ProductPage({
  ...
  return (
    <Layout>
      <Head>
        <title>{content?.pageInfo?.title || "제품 소개 "}</title>
      </Head>
      <main className="min-h-screen bg-white pt-[90px] text-slate-900">
        <HeroSection
          title={content?.pageInfo?.title || "제품 소개"}
          backgroundImage={
        ...
        <div className="relative z-30 -mt-8 sm:-mt-10">
          <BreadcrumbSection
            path={content?.pageInfo?.pageLocation || "사업분야 > 제품 소개"}
          />
        </div>
        ...
        {section && (
          <section className="relative z-0 bg-gradient-to-b from-slate-100/50 via-[#f0f5ff] to-slate-50/60 px-4 pb-20 pt-20 md:pb-20 md:pt-20">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
              ...
              >
                {content?.section1?.title || "제품 소개"}
              </motion.h2>

              {/* Tagline: subtitle sizing like R&D subtitle */}
              <motion.p
                className="text-slate-600 text-base md:text-lg leading-relaxed max-w-3xl whitespace-pre-line"
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, amount: 0.3 }}
                variants={leftAlignTextVariants}
              >
                {content?.section1?.description || "제품 소개"}
              </motion.p>
              ...
              >
                {content?.section1?.products.map((product, index) => (
                  <motion.div
                    key={index}
              ...
              <motion.p
                ...
                variants={leftAlignTextVariants}
              >
                {content?.section1?.footnote || "제품 소개"}
              </motion.p>
            </div>
          </section>
        )}
